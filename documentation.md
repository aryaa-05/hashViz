...
- **FNV-1a (64-bit)**
  - **How it works:** A slight variation of FNV-1. It reverses the core loop: it first XORs the input byte, _then_ multiplies by the FNV prime.
...
3.  Executes the hash function **5,000 times** in a tight loop.
    - _Note: Inside the loop, the input is slightly mutated (`input1 + i`) to prevent modern JavaScript engines (like V8) from optimizing away identical function calls._
